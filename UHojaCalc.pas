...
BEGIN
  ...
  m_eTipo := thcNone;
  // find .XLS*
  IF (system.pos('.XLS', UpperCase(ExtractFileExt(m_strFileName))) = 1) THEN
...
BEGIN
  ...
  BEGIN
    ...
    // m_vPrograma.WorkBooks.Open(m_strFileName);
    m_vPrograma.Visible := Visible;
    m_vDocument := m_vPrograma.ActiveWorkBook;
    m_vActiveSheet := m_vDocument.ActiveSheet;
  END { IF };
